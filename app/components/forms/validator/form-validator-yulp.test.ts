...
import { object, string, number, date, boolean, array, ref, setLocale } from 'yup';
...
describe("MaxFriseFormValidator", () => {
    test("validates schema", async () => {
        ...
        expect(result).toStrictEqual({
            numbers: [
                undefined, 'numbers[1] must be a `number` type, but the final value was: `"hola"`.', 'numbers[2] must be a `number` type, but the final value was: `"adios"`.', undefined, 'numbers[4] must be a `number` type, but the final value was: `"bye"`.'
            ],
